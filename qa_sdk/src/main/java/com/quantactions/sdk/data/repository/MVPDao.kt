...
interface MVPDao {
    ...
    @Query("SELECT * from trend_table WHERE trend = :trendCode and timestamp >= :from and timestamp <= :to ORDER BY timestamp DESC")
    ...
    @Query("SELECT * from sleep_summary_table where timestamp >= :from and timestamp <= :to ORDER BY timestamp DESC")
